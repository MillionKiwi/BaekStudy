...
public class LadderPortal_Corrected {
    ...
    public static Location findNextLoc(Location loc) {
        int cur_x = loc.x; // 현재 열
        int cur_y = loc.y; // 현재 행
        ...
        for (int i = 0; i < N; i++) { // 행 탐색
            for (int j = 0; j < M; j++) { // 열 탐색
                if (i == cur_y && j == cur_x) continue;

                if (cur_char == ladder[i][j]) {
